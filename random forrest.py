...
def game(number_of_games):
    ...
    while x < number_of_games:
        ...
        for hand in hands:
            ...
            while not hand.finished:
                ...
                if not hand.finished:
                    ...
                    print('tot:',int(hand.total), '| dealer:',str(dealer.cards[0].value),'| soft:', hand.check_soft(),'| s.plit:', hand.can_split)
        ...
        if not game_over:
            for hand in hands:
                if dealer.total > hand.total:
                    ...
                    print('DRAW')
                    hand.score = 0
                '''print('hand.score = ',hand.score)
                your_score += hand.score
                print(hand.total," : ",hand.score)'''
        game_scre = 0
        for hand in hands:
            print('hand.score = ', hand.score)
            game_scre += hand.score
            your_score += game_scre
            print(hand.total, " : ", hand.score)

        your_score_tot.append(your_score)

        game_score = your_score_tot[-1] - your_score_tot[-2]
        game_score = game_scre
        ...
        if game_score > 0:
            if not bj:
                print(game_memory)
                while game_memory[-1][-1] == 'h': # removes hands that loose after splitting
                    game_memory.pop(-1)
                print([game_memory, game_score])
...
def train_model(x, y, model=False):
